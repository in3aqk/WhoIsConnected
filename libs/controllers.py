...
from twisted.web.template import Element, XMLString, XMLFile, renderer, tags
...
class Pages():
    ...
    def __init__(self):
        self.conf = readIni()
        self.html_path = os.path.dirname(os.path.realpath(__file__)) + "/../html/"
        ...
        for line in page.splitlines():
            if "{{" in line:
                ...
                if m:
                    found = m.group(1)
                    if found == "content":
                        insert_page = self.__get_page(page_name)
